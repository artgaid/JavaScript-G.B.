...
function init() {
    ...
    */
    ...
    total.innerHTML = "Ваш счет : " + score;
}
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        var total = document.getElementById("total-score");
        total.innerText = "Ваш счет : " + score;

    } else if (unit_classes.includes('wall-unit')) {
        check = false;
        finishTheGame();
    }
    return check;
...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        } else if (!food_cell_classes.includes('wall-unit')) {
            var classes = '';
            for (var i = 0; i < food_cell_classes.length; i++) {
                classes += food_cell_classes[i] + ' ';
            }
...
function createWall() {
    ...
    while (!wallCreated) { //пока препядствие не создали
        // рандом
